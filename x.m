...
% PLOT_MODULOGRAM_FIGURE
%   Draws a heat-map of the amplitude-phase distribution (top)
%   and two modulation-index curves (bottom):
%       • MI_sin — based on a sinusoidal fit
%       • MI_KL  — KL-divergence from a uniform distribution
%
% INPUTS
%   AmplP_matrix     [nFreq × nBins] aggregated amplitude-phase matrix
%   ~                (legacy) unused single-MI vector
%   binCenters       [1 × nBins] phase-bin centres (radians)
%   freq_array       [nFreq × 1] gamma centre frequencies (Hz)
%   outFile          path to PNG file to be written
%   anatomicalRegion text label for electrode location
%
%   Author: ChatGPT auto-fix • May 2025
% -------------------------------------------------------------------------
...
for iF = 1:nFreq
    ...
    fit_vals = X*params;
    rho_sin  = sqrt(mean((y - fit_vals).^2));
    MI_sin(iF) = 1 - rho_sin / std(y);
    
    % --- KL-divergence MI ---
    if any(A_gamma)
        p = A_gamma / sum(A_gamma);
        MI_KL(iF) = sum(p .* log((p+eps)./(uniformDist+eps))) / log(nBins);
    end
end

% ---------- 3) Plot MI curves -------------------------------------------
subplot(2,1,2); hold on;
plot(freq_array, MI_sin, '-o',  'LineWidth',2,'DisplayName','MI_{sin}');
plot(freq_array, MI_KL,  '--s', 'LineWidth',2,'DisplayName','MI_{KL}');
xlabel('\gamma centre frequency (Hz)');
ylabel('Modulation Index');
legend('Location','best'); grid on;
title('Modulation Indices across \gamma sub-bands');
hold off;
