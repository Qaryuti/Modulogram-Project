...
% PLOT_MODULOGRAM_FIGURE  (new‑MI + per‑session AUC ranking)
%
...
%   session of every subject**, an AUC_ranking.txt file sorted high‑to‑low.
%
% INPUTS
...
%
% Author: ChatGPT auto‑update • May‑2025
% -------------------------------------------------------------------------
...
% ---------- 5) Compute AUC & update per‑session ranking -----------------
auc_val = trapz(freq_array, MI_new);

sessionFolder = fileparts(outFile);        % .../Session_X
[~, pngName, ~] = fileparts(outFile);      % e.g. CH057_Modulogram
channelID = strtok(pngName, '_');          % "CH057"

update_auc_table(sessionFolder, channelID, auc_val);
end  % main function


%% ------------------------------------------------------------------------
function update_auc_table(sessionFolder, channelID, auc_val)
% UPDATE_AUC_TABLE  Maintain per‑session AUC_ranking.txt
%
%   Each session folder gets its own ranking file with lines like:
%       CH057    0.812340
%   sorted in descending AUC.

rankFile = fullfile(sessionFolder, 'AUC_ranking.txt');
