function x4(config)
    ...
    % This wrapper loops over subjects, for a specified session, alignment, and channel.
    ...
    for subjIdx = 1:numel(config.subjects)
        ...
        % Save the allData struct for this subject after all channels/alignments
...
% === Recommended FIR Filtering Block for Smooth Surface ===
stepSize = 3;  % Hz step between center frequencies
bandwidth = 7.5;
...
fir_order = 1000;  % high order for tight, clean filters

for iG = 1:numGammaBands
    cfreq = centerFreqs(iG);
    lowBound = max(cfreq - bandwidth, 1);  % avoid 0 Hz
    highBound = min(cfreq + bandwidth, Fs/2 - 1);  % avoid Nyquist
...
% Normalize each sub-band so that the mean of each row is 1.
...
% Retain the per-sub-band results.
finalAggAmplP = normalized_aggAmplP_sub;  % [numGammaBands x numBins]
finalAggMI = aggMI_sub;                   % [numGammaBands x 1]

    %% Debug: Print Modulation Analysis Summary (per sub-band)
    fprintf('--- Modulation Analysis Summary ---\n');
    fprintf('Number of gamma sub-bands: %d\n', numGammaBands);
    fprintf('Number of trials: %d\n', nTrials);
    fprintf('Size of allAmplP_sub (subBand x trial x numBins): [%d %d %d]\n', size(allAmplP_sub));
    fprintf('Size of allMI_sub (subBand x trial): [%d %d]\n', size(allMI_sub));
    fprintf('Size of aggregated amplitude-phase per sub-band (aggAmplP_sub): [%d %d]\n', size(aggAmplP_sub));
    fprintf('Aggregated MI per sub-band: %s\n', mat2str(aggMI_sub, 4));
    fprintf('Final aggregated amplitude-phase matrix (finalAggAmplP): [%d %d]\n', size(finalAggAmplP));
    fprintf('Final aggregated MI (per sub-band): %s\n', mat2str(finalAggMI, 4));
    fprintf('--- End of Modulation Analysis Summary ---\n');
...
fprintf('Final aggregated AmplP: %s\n', mat2str(finalAggAmplP, 4));
fprintf('Final aggregated MI: %f\n', finalAggMI);
...
fprintf('Modulogram computed and saved for %s, Session %d, Channel %s\n', subjectID, sessionNum, channelLabel);
...
function [AmplP_matrix, MI_vector, binCenters, freq_array] = compute_modulogram_analysis(pac_signals, Fs, num_bins)
    ...
    %   freq_array   - the center frequencies from pac_signals
        ...
        for iF = 1:nFreq
            ...
            % Fit a first-order sinusoid to A_gamma(phi_theta)
        phi = binCenters; % phase bins: size [1 x num_bins]
        A_gamma = amplP;  % observed amplitudes: size [1 x num_bins]

        % Construct design matrix for sinusoidal regression
        ...
        % Compute RMS error (rho_sin)
        rho_sin = sqrt(mean((y - fit_vals).^2));
        MI_sin = 1 - (rho_sin / std(y));  % Normalized inverse error

        % Compute KL divergence (raw MI)
        uniformDist = ones(1, num_bins) / num_bins;
        if sum(A_gamma) > 0
            p = A_gamma / sum(A_gamma);
        else
            p = A_gamma;
        end
        MI_kl = sum(p .* log((p + eps) ./ (uniformDist + eps)));
        MI_kl = MI_kl / log(num_bins);

        % Store both MI values
        MI_vector(iF) = MI_sin;      % sinusoidal-based MI
        MI_vector_KL(iF) = MI_kl;    % KL-divergence-based MI
        end        
end
...
% PLOT_MODULOGRAM_FIGURE  (single‑file version)
%
%   Draws a modulogram (heat‑map) and a single modulation‑index (MI_new)
%   curve derived from a sinusoidal fit.  After plotting, it computes the
%   area‑under‑the‑curve (AUC) of the MI curve and maintains, for every
%   subject, an AUC‑ranking text file sorted in descending order.
%
% INPUTS
%   AmplP_matrix     [nFreq × nBins] aggregated amplitude‑phase matrix
%   ~                (legacy) unused
%   binCenters       [1 × nBins] phase‑bin centres (radians)
%   freq_array       [nFreq × 1] gamma centre frequencies (Hz)
%   outFile          full PNG path for the modulogram figure
%   anatomicalRegion text label for the electrode location
%
% -------------------------------------------------------------------------
...
axis xy; colorbar;
xlabel('\theta phase (rad)'); ylabel('\gamma centre frequency (Hz)');
title(sprintf('Modulogram – %s', anatomicalRegion));

% ---------- 2) Compute MI curve -----------------------------------------
[nFreq, nBins] = size(AmplP_matrix);
MI_new  = zeros(nFreq,1);
phi = binCenters(:);                       % column vector
X = [ones(nBins,1) cos(phi) sin(phi)];     % design matrix
...
% ---------- 5) Compute AUC & update ranking -----------------------------
auc_val = trapz(freq_array, MI_new);

% Determine subject folder from PNG path:  .../<Subject>/Session_X/<PNG>
sessionFolder = fileparts(outFile);
subjectFolder = fileparts(sessionFolder);

[~, pngName, ~] = fileparts(outFile);
channelID = strtok(pngName, '_');  % e.g. "CH057"
...
%   Each line:  CH057    0.812340   (tab‑delimited)  sorted descending
